...
axios.interceptors.request.use(
  config => {
    ...
    // if (router.app._route.fullPath.indexOf('/submit_order') !== -1) store.state.has_loading = true;
...
axios.interceptors.response.use(
  response => {
    // console.log(response,'config222222222222222222')
    ...
    if (router.app._route.fullPath.indexOf('/business_shop') !== -1) {
      ...
      if (response.config.url.indexOf('/getSellerGoods') !== -1) {
        ...
        // console.log('getSellerGoods',store.state.shop_loading)
      }
    } else {
    ...
    // console.log(store.state.has_loading,'router.app._route.fullPath2')
    return response;
  },
  error => {
    store.state.has_loading = false;
