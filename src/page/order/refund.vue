    ...
    <div class="refund_body" :style="{top: header_height}" :class="{foucus_target: foucus_target==0}">
      <div class="refund_box">
        ...
        <div class="remarks" v-show="notes">
          ...
          <div>
            <textarea v-model="remarks_content" @focus="focus_fn" @blur="on_blur" maxlength="50" resize="none" :placeholder="apply_refund.supplementary"></textarea>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        refund_resion_list: [
          ...
          { resion: this.$t('apply_refund.no_promise_discount'), is_checked: false },
          { resion: this.$t('apply_refund.consignee_information'), is_checked: false },
          { resion: this.$t('apply_refund.slow_delivery_rate'), is_checked: false },
    ...
    methods: {
      focus_fn() {
        console.log(document.querySelector('.phoneHeader').offsetHeight);
        this.header_height = document.querySelector('.phoneHeader').offsetHeight + 'px';
        this.foucus_target = 0;
        console.log(this.foucus_target)
      },
      on_blur() {
      ...
      // 申请退款
      toApplyForARefund() {
        console.log(this.remarks_content)
        store.dispatch('toApplyForARefund', {
          orderId: this.$route.query.id,
          remark: this.remarks
        }).then(res => {
          console.log(res.data.code);
          if (res.data.code == 1) {
            this.$router.push({ path: '/refund_details', query: { id: this.$route.query.id } });
