<template>
  <div class="orderList">
    <div class="contain" id="scrollWrap">
      ...
      <div class="tab_list" @mousemove="mousemove">
      ...
      <div class="order_list" :class="{iphone5: isIphone5, iphone6: isIphone6, iphoneP: isIphoneP, iphoneX: isIphoneX}" v-if="!not_logged">
        ...
        <div class="order_item" v-for="(item, index) in order_list" :key="index">
          ...
          <div class="btn_area">
            <span style="color: #5BA6FF; position: absolute; left: 0; font-size: 14px; border: 1px solid white;" v-if="item.rerund_status==2">
...
export default {
  ...
  methods: {
    ...
    scrollList() {
      ...
      if (this.currentPage >= store.state.order_list.totalPage) {
        store.state.order_loading = false
      ...
      if (scrollTop + clientHeight - scrollHeight >= -50) {
        this.currentPage++
        document.querySelector('.tab_list').style.position = 'fixed'
        store
          .dispatch('orderList', {
            status: this.select_order_status,
            currentPage: this.currentPage,
