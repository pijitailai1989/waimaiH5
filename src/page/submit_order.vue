    ...
    <div class="order_contain">
      ...
      <div v-if="isHaveAddr" class="order_address" @click="orderAddr(sellerId)">
        ...
        <div class="post_detail">
          <p class="post_addr">{{first_address.address}}</p>
      ...
      <div class="order_detail" v-for="(items, index) in carsData" :key="index">
        ...
        <div class="goods_item" v-for="(item, index) in items.sellerList" :key="index">
          ...
          <div class="goods_details">
            <p class="goods_name">
              ...
              <span class="goods_money">{{item.money}}P</span></p>
            <p class="goods_format">x{{item.number}} <span v-if="item.specs"> {{item.specs}}</span> <span v-if="item.label">/ {{item.label}}</span></p>
          </div>
        </div>
        <!-- 配送费 -->
        ...
        <div class="all_amount">
          {{get_en.to_cope_with}} <span>{{final_money}}P</span>
        </div>
      </div>
      <!-- 备注 支付方式 -->
      <div class="tip_contain">
        ...
        <div class="order_note">
          ...
          <textarea v-model="remark" :placeholder="get_en.order_tip"></textarea>
        </div>
      </div>
    </div>
    <div class="footer">
      <div class="all_money">
        {{get_en.total}} 
        <span class="total_money" v-show="final_money">{{final_money}}P&nbsp;</span>
...
export default {
  ...
  methods: {
    ...
    selectType() {
      ...
      if (this.showCoupon) {
        this.discountMoney = parseFloat(this.select_coupon.money);
        console.log("discountMoney", this.select_coupon.money);
        this.final_money = (
      ...
      store
        ...
        .then(res => {
          ...
          if (this.reduction.fullReduction !== 0) {
            ...
            this.others_money = this.first_address
              ? parseFloat(this.others_money) +
                parseFloat(this.first_address.logistics)
              : parseFloat(this.others_money);
          }
          this.totalPrice = this.first_address
            ? (
          ...
          if (this.showCoupon) {
            ...
            this.final_money = this.formatCount(this.final_money);
            console.log(this.final_money, "this.final_money");
          } else {
