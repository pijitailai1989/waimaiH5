<template>
	<div class="contain">
		...
		<div>
			...
			<!-- 优惠券 -->
			<div class="home_coupon_box" @click="goCoupon">
...
export default {
  ...
  created() {
    ...
    if (
      ...
      this.current_city = JSON.parse(
        localStorage.getItem('current_address_latlng')
      ).name
    } else {
      if (this.lang == 'zh') {
        this.current_city = '定位中...'
      } else if (this.lang == 'en') {
        this.current_city = 'Position in...'
      }
    }

    sessionStorage.setItem('new_current_city', this.current_city)
  ...
  mounted() {
    if (localStorage.getItem('current_address_latlng')) {
      ...
      this.current_city_address_name = JSON.parse(
        localStorage.getItem('current_address_latlng')
      ).name
      this.getCity({
        lat: this.lat,
  ...
  methods: {
    ...
    to_label(catId, title, item) {
      ...
      catId =
        item.sellerLabelPoint == 1 ? '' : localStorage.setItem('catId', catId)
      let allDay = ''
      let deliveryScheme = ''
    ...
    errorPosition() {
      ...
      self.current_city = self.$t('choose_address.locate_failure')
    },
    showPosition(position) {
      let select_address = localStorage.getItem('current_address_latlng')
      if (select_address) {
        this.lat = parseFloat(JSON.parse(select_address).lat)
        this.lng = parseFloat(JSON.parse(select_address).lng)
        this.current_city_address_name = JSON.parse(select_address).name
        // 定位当前位置
        this.getCity({
          lat: this.lat,
          lng: this.lng
        ...
        localStorage.setItem('current_address_latlng', JSON.stringify(this.obj))
        // 定位当前位置
        this.getCity({
          lat: this.lat,
          lng: this.lng
      ...
      self.geocoder.geocode(
        ...
        function(results, status) {
          ...
          if (status == 'OK') {
            ...
            let current_city_address_name = localStorage.getItem(
              'current_address_latlng'
            )
            if (self.current_city_address_name) {
              self.current_city = self.current_city_address_name
            } else {
              self.current_city = neighborhood_all
            }
            setCookie('current_city', self.current_city)
            if (
              JSON.parse(localStorage.getItem('current_address_latlng')).name
            ) {
              ...
              sessionStorage.setItem('current_city_address', neighborhood_all)
            }
          } else {
