			...
			<div class="seller_list" v-for="(item, index) in seller_list" :key="index">
				<div class="seller_list_top clearFloat" @click="to_detail(item.id)">
					<div class="seller_header">
						<img :src="baseUrl + item.sellerLogo" @error="imgError(index)">
...
export default {
  ...
  watch: {
    obj: function(newVal, oldVal) {
      ...
      this.lat = parseFloat(JSON.parse(this.current_address_latlng).lat)
      this.lng = parseFloat(JSON.parse(this.current_address_latlng).lng)
      this.render_list()
    }
  },
  computed: {
  ...
  created() {
    ...
    if (this.current_address_latlng) {
      ...
      this.lng = parseFloat(JSON.parse(this.current_address_latlng).lng)
      if (
        localStorage.getItem('label_title') == '龙配送' ||
        localStorage.getItem('label_title') == 'LOONG'
      ) {
  ...
  methods: {
    ...
    imgError(index) {
      store.state.seller_list.sellerList[
        index
      ].sellerLogo = require('../../../assets/images/mo.png')
    },
    to_detail(id) {
    ...
    scrollList() {
      if (localStorage.getItem('current_address_latlng')) {
        this.lat = parseFloat(JSON.parse(localStorage.getItem('current_address_latlng')).lat)
        this.lng = parseFloat(JSON.parse(localStorage.getItem('current_address_latlng')).lng)
      }
      ...
      if (this.currentPage >= store.state.seller_list.totalPage) {
        store.state.loading = false
        return
      } else {
        store.state.loading = true
      ...
      console.log(clientHeight+ ':' +  scrollTop+ ':' +seller_list_top_box);
      ...
      let speed = 0
      if (this.$router.name == 'home') speed = -50
      ...
      store
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            let item = store.state.seller_list.sellerList
            item.forEach(element => {
              if (element.sellerActivityList) {
                element.sellerActivityList.sort(function(a, b) {
                  return b.collectNum - a.collectNum
                })
              }
            })
            Object.assign(store.state.seller_list, item)
          }
        })
        .catch(err => {
          console.error('err:', err)
