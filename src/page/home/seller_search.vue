...
<script>
  ...
  export default {
    ...
    methods: {
      focused() {
        this.searchHeight = parseFloat(getComputedStyle(document.querySelector('.search_header')).height) + 'px'
      },
      // 选择历史搜索
          ...
          store
            ...
            .then(res => {
              ...
              if (res.data.code == 1) {
                ...
                for (let i = 0; i < data.data.sellerList.length; i++) {
                  if (data.data.sellerList[i].goodList && data.data.sellerList[i].goodList.length > 2) {
      ...
      go_back() {
        ...
        if (sessionStorage.getItem('list_to_search') && sessionStorage.getItem('list_to_search') == 'true') {
